...
describe("GET /api/auth/error", () => {
	it("uses BETTER_AUTH_URL when configured", async () => {
		vi.resetModules();
		vi.doMock("@/config", () => ({
			default: { BETTER_AUTH_URL: "https://smartdeploy.example.com", WAITING_LIST_ENABLED: true },
		}));
	...
	it("falls back to forwarded host/protocol", async () => {
		vi.resetModules();
		vi.doMock("@/config", () => ({
			default: { BETTER_AUTH_URL: "http://localhost:3000", WAITING_LIST_ENABLED: true },
		}));
	...
	it("routes to /auth when waiting list is disabled", async () => {
		vi.resetModules();
		vi.doMock("@/config", () => ({
			default: { BETTER_AUTH_URL: "https://smartdeploy.example.com", WAITING_LIST_ENABLED: false },
		}));
		...
		expect(res.headers.get("location")).toBe("https://smartdeploy.example.com/auth");
	});
});
