...
import { SCAN_PROGRESS_NODES } from "@/components/scan-progress/scanProgressState";

type ScanProgressNodeListProps = {
	activeNode: string;
	completedNodes: string[];
	failedNode: string | null;
};

export function ScanProgressNodeList({ activeNode, completedNodes, failedNode }: ScanProgressNodeListProps) {
	return (
		<div className="w-full lg:w-1/3 flex flex-col gap-4">
			<h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Workflow Chain</h3>
			<div className="flex flex-col gap-3">
				{SCAN_PROGRESS_NODES.map((node) => {
