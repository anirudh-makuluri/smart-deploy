...
export default async function AdminAgentEvalsPage({ searchParams }: AgentEvalsPageProps) {
	...
	const helpfulCount = countWhere(reviewedRuns, (run) => displayHelpfulness(reviewsByRunId.get(run.runId) ?? null) === "helpful");
	const partialCount = countWhere(reviewedRuns, (run) => displayHelpfulness(reviewsByRunId.get(run.runId) ?? null) === "partially_helpful");
	const notHelpfulCount = countWhere(reviewedRuns, (run) => displayHelpfulness(reviewsByRunId.get(run.runId) ?? null) === "not_helpful");
	const judgedCount = countWhere(reviewedRuns, (run) => Boolean(reviewsByRunId.get(run.runId)?.judgedAt));
	const missingToolSuggestions = countWhere(runs, (run) => run.suggestedFailureMode === "missing_tool");
	const toolLimitSuggestions = countWhere(runs, (run) => run.suggestedFailureMode === "tool_limit");
		...
		<section className="space-y-5">
			...
			<div className="grid gap-5">
				{runs.length === 0 ? (
					...
					runs.map((run) => {
						const review = reviewsByRunId.get(run.runId) ?? null;
						return (
							<Card key={run.runId} className="rounded-lg border-white/10 bg-card/80">
