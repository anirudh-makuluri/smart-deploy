...
export function normalizeScanResultPayload(value: unknown): SDArtifactsResponse | null {
	try {
		...
		return {
			...
			stack_summary: typeof p.stack_summary === "string" ? p.stack_summary : base.stack_summary,
			...
			dockerfiles: (p.dockerfiles && typeof p.dockerfiles === "object")
				? Object.fromEntries(
					Object.entries(p.dockerfiles as Record<string, unknown>).filter(([, v]) => typeof v === "string") as Array<[string, string]>
				)
				: base.dockerfiles,
			docker_compose: typeof p.docker_compose === "string" ? p.docker_compose : null,
			nginx_conf: typeof p.nginx_conf === "string" ? p.nginx_conf : null,
